...
const Calculator = () => {
    ...
    const btnClick = (item) => {
        ...
        if (item.action === BTN_ACTIONS.CALC) {
            if (expression.trim().length <= 0) return;  /* se expressão vazia retorna nada */

            expDiv.parentNode.querySelector('div:last-child').remove(); 

            const cloneNode = expDiv.cloneNode(true); /* clona e adiciona a ref na dv de cma */
            ...
            try {
                ...
                setTimeout(() => {
                    cloneNode.style.transform = transform;
                    cloneNode.innerHTML = 'Erro de sintaxe';
